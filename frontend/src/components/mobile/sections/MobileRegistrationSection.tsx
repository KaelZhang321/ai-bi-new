...
const columns = [
  ...
  { title: '千万(到)', dataIndex: 'qianwan_arrive', key: 'qa', width: 62, render: (v: number) => <span style={{ color: v > 0 ? theme.colors.accentGreen : theme.colors.textSecondary, fontWeight: v > 0 ? 600 : 400 }}>{v}</span> },
  { title: '百万(报)', dataIndex: 'baiwan_register', key: 'br', width: 62 },
  { title: '百万(到)', dataIndex: 'baiwan_arrive', key: 'ba', width: 62, render: (v: number) => <span style={{ color: v > 0 ? theme.colors.accentGreen : theme.colors.textSecondary, fontWeight: v > 0 ? 600 : 400 }}>{v}</span> },
  { title: '总报名', dataIndex: 'total_register', key: 'tr', width: 56, render: (v: number) => <strong style={{ color: theme.colors.accentCyan }}>{v}</strong> },
  { title: '总抵达', dataIndex: 'total_arrive', key: 'ta', width: 56, render: (v: number) => <strong style={{ color: theme.colors.accentGreen }}>{v}</strong> },
]

const detailColumns = [
  { title: '客户姓名', dataIndex: 'customer_name', key: 'name', width: 80 },
  { title: '签到状态', dataIndex: 'sign_in_status', key: 'status', width: 70, render: (v: string) => <span style={{ color: v === '已签到' ? theme.colors.accentGreen : theme.colors.textSecondary }}>{v || '-'}</span> },
...
const MobileRegistrationSection: React.FC = () => {
  ...
  return (
    <div>
      <MobileSectionTitle title="报名 VS 签到" subtitle="各区域按金额等级对比" accentColor={theme.colors.accentCyan} />
      <MobileCard title="报名/抵达统计" subtitle="点击柱体查看明细" glowColor={theme.colors.accentCyan}>
        <StackedBarChart categories={categories} series={series} height={260} onBarClick={handleChartClick} />
      </MobileCard>
      <div style={{ height: 12 }} />
      <MobileCard title="金额等级矩阵" glowColor={theme.colors.accentCyan}>
