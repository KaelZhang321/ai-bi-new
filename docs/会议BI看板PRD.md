...
- 鼠标悬停显示具体数值
- 点击柱体可下钻查看明细（弹出 Modal 展示该区域+该等级的客户列表，包含字段：客户姓名、签到状态、客户类别、门票类型、店铺来源）
- 图例可点击切换显示/隐藏

**3.3.1.6 下钻明细接口**

```
GET /v1/registration/detail?region=xxx&level=xxx
```

返回该区域+该等级的客户明细列表，支持 region 和 level（金额等级）筛选参数。
...
- 点击柱体下钻：弹出 Modal 展示该区域目标客户明细（客户姓名、客户等级、成交低限、成交高限、铺垫成熟度、新老客户、是否抵达）

**3.5.2.5 下钻明细接口**

```
GET /v1/source/target-detail?region=xxx
```
...
- 点击柱体下钻：弹出 Modal 展示该区域的成交明细（客户姓名、分公司、成交类型、成交内容、新成交金额、收款金额、方案类型）
- 支持从大区下钻至分公司、客户类型、新旧客户、门票类型等多个维度

**3.7.1.5 下钻明细接口**

```
GET /v1/achievement/detail?region=xxx
```
...
- 点击达成数量可下钻：弹出 Modal 展示该区域+该方案的成交明细（客户姓名、成交内容、新成交金额、收款金额、记录日期）

**3.9.1.5 下钻明细接口**

```
GET /v1/proposal/detail?region=xxx&proposal_type=xxx
```
